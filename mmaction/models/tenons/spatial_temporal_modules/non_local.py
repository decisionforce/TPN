...
class NonLocalModule(nn.Module):
    def __init__(self, in_channels=1024, nonlocal_type="gaussian", dim=3, embed=True, embed_dim=None, sub_sample=True,
                 use_bn=True):
        ...
        if self.embed:
            if dim == 2:
                ...
                self.theta = nn.Conv3d(in_channels, self.embed_dim, kernel_size=(1, 1, 1), stride=(1, 1, 1),
                                       padding=(0, 0, 0))
                self.phi = nn.Conv3d(in_channels, self.embed_dim, kernel_size=(1, 1, 1), stride=(1, 1, 1),
                                     padding=(0, 0, 0))
                self.g = nn.Conv3d(in_channels, self.embed_dim, kernel_size=(1, 1, 1), stride=(1, 1, 1),
                                   padding=(0, 0, 0))
        ...
        if self.nonlocal_type == 'gaussian':
            ...
            if dim == 2:
                self.concat_proj = nn.Sequential(
                    nn.Conv2d(self.embed_dim * 2, 1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)),
                    nn.ReLU())
            elif dim == 3:
                self.concat_proj = nn.Sequential(
                    nn.Conv3d(self.embed_dim * 2, 1, kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0)),
                    nn.ReLU())
